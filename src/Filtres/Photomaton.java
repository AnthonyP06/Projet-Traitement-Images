/**
 * Filtre appliquant un effet photomaton à la photo. Le filtre photomaton produit une mosaïque de 4 images identiques à l'image initiale.
 * 
	...
	public BufferedImage appliquerFiltre(BufferedImage image)
	{
		...
		for (int y = 0; y < image.getHeight()-1; y=y+2)		// On se déplace suivant les y de 2 pixels en 2 pixels
		{
			for (int x=0; x < image.getWidth()-1; x=x+2)		// On se déplace suivant les x de 2 pixels en 2 pixels
			{
				...
				//On colorie les 4 parties de l'image traitée de la même couleur   
				traite.setRGB(x/2, y/2, coloration.getRGB());			//En haut à gauche
				traite.setRGB(x/2+(int)(traite.getWidth()/2), y/2, coloration.getRGB());	 //En haut à droite
				traite.setRGB(x/2, y/2+(int)(traite.getHeight()/2), coloration.getRGB());    //En bas à gauche
				traite.setRGB(x/2+(int)(traite.getWidth()/2), y/2+(int)(traite.getHeight()/2), coloration.getRGB());    //En bas à droite
			}
		}
		return traite;
	}
